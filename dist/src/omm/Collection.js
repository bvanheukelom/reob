...
var Collection = (function () {
    ...
    Collection.prototype.updateOnce = function (sp, updateFunction, attempt) {
        ...
        var updatePromise = Promise.all([objectPromise, currentSerialPromise, resultPromise, rootObjectPromise]).then(function (values) {
            ...
            return omm_event.callEventListeners(_this.getEntityClass(), "preSave", ctx).then(function () {
                ...
                if (ctx.cancelledWithError()) {
                    ...
                    var resultPromise = _this.update(sp, function (subObject) {
                        ...
                        return _this.sendEventsCollectedDuringUpdate(r.object, r.object, r.rootObject, functionName, sp, r.events, ud).then(function () {
                            var ctx = new omm.EventContext(r.rootObject, _this); // this is being emitted on the collection level so it needs to look as if the object was updated is the root element
                            ctx.functionName = functionName;
                            ctx.serializationPath = sp;
