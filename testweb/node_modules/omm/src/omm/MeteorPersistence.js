...
function callHelper(o, callback) {
    ...
    omm_annotation.PersistenceAnnotation.getMethodFunctionNames(c.prototype).forEach(function (functionName) {
        ...
        helper[methodOptions.functionName] = function () {
            ...
            var callOptions = omm_annotation.PersistenceAnnotation.getMethodOptions(functionName);
...
function staticCallHelper(tc, callback) {
    ...
    // static functions
    var helper = {};
    var className = omm_annotation.className(tc);
    omm_annotation.PersistenceAnnotation.getMethodFunctionNamesByObject(tc).forEach(function (functionName) {
        var methodOptions = omm_annotation.PersistenceAnnotation.getMethodOptions(functionName);
...
exports.staticCallHelper = staticCallHelper;
...
var MeteorPersistence = (function () {
    ...
    MeteorPersistence.init = function () {
        if (!MeteorPersistence.initialized) {
            MeteorPersistence.meteorObjectRetriever = new MeteorObjectRetriever_1.default();
            MeteorPersistence.serializer = new Serializer_1.default(MeteorPersistence.meteorObjectRetriever);
            Serializer_1.default.init();
    ...
    MeteorPersistence.createMeteorMethod = function (options) {
        ...
        if (Config.getMeteor().isServer || typeof options.serverOnly == "undefined" || !options.serverOnly) {
            ...
            m[meteorMethodName] = function () {
                ...
                Status_1.default.methodContext = this;
                ...
                try {
                    ...
                    if (!isStatic) {
                        if (!staticObject) {
                            ...
                            if (options.parentObject && options.parentObject.constructor) {
                                var className = omm_annotation.className(options.parentObject.constructor);
                            ...
                            if (typeof staticObject == "string") {
                                object = getRegisteredObject(staticObject);
                                if (!object)
                    ...
                    try {
                        if (callbackIndex != -1) {
                            var syncFunction = Config.getMeteor().wrapAsync(function (ascb) {
                    ...
                    if (!theError) {
                        ...
                        var doc = MeteorPersistence.serializer.toDocument(result);
                        ...
                        if (Array.isArray(result)) {
                            ...
                            for (var ri = 0; ri < result.length; ri++) {
                                var t = omm_annotation.PersistenceAnnotation.getClass(result[ri]);
                            ...
                            var t = omm_annotation.PersistenceAnnotation.getClass(result);
                            if (t && omm_annotation.className(t) && omm_annotation.PersistenceAnnotation.getEntityClassByName(omm_annotation.className(t))) {
        ...
        if (options.replaceWithCall) {
            MeteorPersistence.monkeyPatch(options.parentObject, options.functionName, function (originalFunction) {
                var a = [];
                for (var _i = 1; _i < arguments.length; _i++) {
                    a[_i - 1] = arguments[_i];
                }
                if (!Status_1.default.updateInProgress && (this._serializationPath || options.isStatic || (options.object && typeof options.object == "string"))) {
                    var args = [];
    ...
    MeteorPersistence.wrapClass = function (entityClass) {
        ...
        omm_annotation.PersistenceAnnotation.getCollectionUpdateFunctionNames(entityClass).forEach(function (functionName) {
            MeteorPersistence.monkeyPatch(entityClass.prototype, functionName, function (originalFunction) {
                ...
                if (!Status_1.default.updateInProgress) {
